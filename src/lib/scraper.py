...
def get_column_data(pdf,pageNumber,cropDimArray,locateTables=False,omitRegexp=''):
    pg = pdf.pages[pageNumber]
    ...
    for i in range(len(cropDimArray)):
        ...
        if (locateTables):
            ...
            for j in range(len(textSpaceDelim)):
                        if (omitRegexp == '' or not(bool(re.search(omitRegexp, textSpaceDelim[j])))):
                            data=np.append(data,[[float(textSpaceDelim[j])]],axis=0)
    return data
...
def writeMetaDataToFile(filename,specie,process,units_e,units_sigma,ref,
    ...
    f = open(filename, "x")
    f.write("\t".join(('',specie,process,str(units_e),str(units_sigma),ref,lhsA,lhsB,rhsA,rhsB,str(wavelength),str(lhs_v),str(rhs_v),str(lhs_j),str(rhs_j),background,str(lpu),str(upu))))
    f.close()
